from Util_mol.molecule_tree import annotated_tree_to_mol_tree, Node, tree_to_smiles
from Util_mol.params import avail_atoms, atom_valence, bond_types, bond_valence, prod, MAX_NESTED_BONDS, rule_ranges
...
# attribute mol graph decoder, sometimes called 'tree decoder'
class AttMolGraphDecoder(object):
    ...
    def __init__(self):
        ...
        self.open_rings = {}
        self.same_atom_bonds = defaultdict(set)
    ...
    def rand_rule(self, node, sub_ranges=None):
        ...
        indexes = np.arange(g_range[0], g_range[1])
        if sub_ranges is not None:
            indexes = indexes[sub_ranges]

        assert len(indexes)
        if len(indexes) == 1 and cmd_args.skip_deter:
            result = 0
        else:
            result = self.walker.sample_index_with_mask(node, indexes)  # One hot Builder, result：第几条规则
    ...
    def tree_generator(self, node, left_conn=False, right_conn=False, cap_remain=None, ref_symbol=None, is_last=None):
        ...
        if node.symbol == 'chain':
            ...
            if node.task:
                ...
                candidates = [0]
                pre_idx = node.get_pre()
                if cap_remain > 0 and not (
                        self.maximum_match(pre_idx, cap_remain) == 0 and len(self.open_rings) == MAX_NESTED_BONDS):
                    candidates.append(1)
            ...
            if r == MAX_NESTED_BONDS:  # create new bond
                for i in range(MAX_NESTED_BONDS):
                    if i not in self.open_rings and (
                            (i not in self.same_atom_bonds[self.atom_num - 1]) or cmd_args.bondcompact):
                        bond_idx = i
                        create = True
                        break
                assert create
            ...
            self.same_atom_bonds[self.atom_num - 1].add(bond_idx)
            ...
            if rule <= 1:  # BAH -> hcount charge | charge
                ...
                if cap_remain > 0 and rule == 0:
                    borrow = 1
                cap_remain = self.tree_generator(c, cap_remain=cap_remain - borrow, ref_symbol=ref_symbol,
                                                 is_last=is_last)
                cap_remain += borrow
                node.add_child(c)
            ...
            for i in range(len(p)):
                ...
                if not p[i][0] == '\'':  # non-terminal
                    t = self.tree_generator(c, left_conn, right_conn, cap_remain=cap_remain, ref_symbol=ref_symbol,
                                            is_last=is_last)
